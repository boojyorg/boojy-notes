...
export default function Sidebar({
  ...
  searchMode, searchResults, activeResultIndex,
  navigateResults, clearSearch, handleSearchResultOpen, getActiveResult,
}) {
  ...
  useEffect(() => {
    ...
    const el = sidebarScrollRef.current?.querySelector(`[data-search-index="${activeResultIndex}"]`);
    ...
    <div style={{
      ...
      <div style={{ padding: "8px 10px" }}>
        <div
          ...
          {search && (
            <button onClick={(e) => { e.stopPropagation(); setSearch(""); clearSearch?.(); }}
              style={{
        ...
        <div ref={sidebarScrollRef} style={{ flex: 1, overflow: "auto", padding: "2px 0" }}>
          ...
          {searchResults.groups.map(group => (
            <div key={group.folderId || "_root"}>
              {group.folderName && (
                <div style={{
                  fontSize: 11, color: TEXT.muted, padding: "8px 14px 2px",
              ...
              {group.results.map(result => {
                ...
                return (
                  <button
                    ...
                    style={{
                      width: "calc(100% - 8px)", marginLeft: 5, marginRight: 3,
                      border: "none", outline: "none", appearance: "none", WebkitAppearance: "none",
                      cursor: "pointer",
                    ...
                    onMouseEnter={(e) => { if (!isActive) hBg(e.currentTarget, BG.elevated); }}
                    onMouseLeave={(e) => { if (!isActive) hBg(e.currentTarget, "transparent"); }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      ...
                      <span style={{
                        ...
                        {result.matchIn === "title" ? renderHighlightedTitle(result.title, result.matchStart, result.matchEnd, accentColor) : result.title}
                      </span>
                    </div>
                    {result.snippet ? (
                      <div style={{
                        fontSize: 11, color: TEXT.muted, paddingLeft: 19,
                      ...
                      <div style={{
                        fontSize: 11, color: TEXT.muted, paddingLeft: 19,
                        fontStyle: "italic", lineHeight: "16px",
                      }}>title match</div>
                    ) : null}
                  </button>
                );
...
function renderHighlightedTitle(title, matchStart, matchEnd, accentColor) {
  ...
  return (
    <>
      ...
      <span style={{ color: accentColor, fontWeight: 600 }}>{title.slice(matchStart, matchEnd)}</span>
...
function renderSnippet(snippet, accentColor) {
  ...
  if (highlightStart < 0 || highlightEnd <= highlightStart || highlightStart >= text.length) return text;
  return (
    <>
      {text.slice(0, highlightStart)}
      <span style={{ color: accentColor, fontWeight: 600 }}>{text.slice(highlightStart, highlightEnd)}</span>
