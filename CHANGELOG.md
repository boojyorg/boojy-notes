...
- **The sidebar says what it is for, and Undo is a button** — The column now reads down as three rows before the list: the Notes wordmark with the window's panel toggle, a labelled **New note**, and a quiet **Notes** row carrying Search, New folder and a ··· menu. New note uses a square-and-pen icon in both sidebar states. New note has left the top-right corner, where it sat beside the note's own ··· ; Search has come down from the window's row to the list it searches; and all three list controls are visible at rest rather than appearing on hover, so nothing you use daily has to be found first. New note and the Notes row stay put while the list scrolls. The heading is the plain word *Notes* rather than your storage folder's name, which lives in Settings → Storage beside the control that changes it. Nothing about your files, folders, drag destinations or sort order changed with the label.
