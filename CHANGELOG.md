...
- **Dark mode is a quieter grey, not near-black** — The note used to sit on a near-black, faintly violet ground with a much lighter blue-grey sidebar beside it, so the two read as different materials and white text glared on the sheet; the dimmest text (the *Notes* label, the note's name in the top row, the sidebar's icons) fell below the legibility floor. Dark is now one neutral grey family with small steps, the same territory as Obsidian and Notion: the sheet a shade darker than the sidebar, the selected row a shade lighter again, and the ink a whisper warm rather than blue-white. Every text tier is now readable on every surface it sits on. Light mode is unchanged.
