...
### Improvements
- **The slash menu shows the shortcut you could have typed** — Each row of the `/` menu now carries its typed Markdown shortcut in muted type on the right (`#` beside Heading 1, `-` beside Bullet list, `[]` beside To-do list, `>` beside Quote, `---` beside Divider), the way Notion's does, so the menu teaches the faster route while you use it. Table and Image have no typed shortcut and show nothing; the list is ordered so those two sit at the foot rather than leaving a gap in the column. Two rows are renamed in plain words: Blockquote is now Quote, and Checkbox is now To-do list, which reads with Bullet list and Numbered list as the three kinds of list. Nothing about what the rows insert has changed.
