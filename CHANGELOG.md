...
### Improvements
- **One vault, one tree** — The sidebar's `Folders` and `Notes` sections are gone. In their place a quiet header carries the name of your vault folder, with New note, New folder and a ··· menu at its right, visible in muted ink and full ink when you reach for them. Below it is one list: folders first, alphabetical, then your loose notes in the sort order you chose, exactly as inside a folder. The New note row under the wordmark has moved up into that header (Cmd+N still works); Search stays a row because it becomes the search field in place. Sort by lives in the ··· menu now, with the current mode marked, beside Collapse all folders, Reveal in Finder and Change vault folder. Dragging a note or folder onto the vault header, or into the empty space under the tree, moves it to the root.
